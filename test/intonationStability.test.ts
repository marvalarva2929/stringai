...
{
  const m = scoreIntonationStability(buildNote(440, DUR, sine(25, 5.5)));
...
{
  const m = scoreIntonationStability(buildNote(440, DUR, ramp(80, DUR)));
  console.log(`  score=${m.score} flags=${m.flaggedTimestamps.length} :: "${m.observationSummary}"`);
...
console.log('\nVibrato + drift:');
{
  const pure = scoreIntonationStability(buildNote(440, DUR, sine(25, 5.5))).score;
  const both = scoreIntonationStability(buildNote(440, DUR, (t) => sine(25, 5.5)(t) + ramp(50, DUR)(t)));
...
console.log('\nFlat steady note:');
{
  const m = scoreIntonationStability(buildNote(440, DUR, () => 0, 2, mulberry32(7)));
